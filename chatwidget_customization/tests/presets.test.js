...
const WIDGET_PRESET_DIR = join(here, '..', '..', 'chatwidget_components_lit', 'public', 'clients');
...
describe('bundled client presets', () => {
  ...
  // The same presets are duplicated in chatwidget_components_lit/public/clients
  ...
  it('stays in sync with the widget library copy of each preset', () => {
    ...
    for (const file of presets) {
      ...
      expect(a, `${file} differs between chatwidget_customization and chatwidget_components_lit`).toEqual(b);
    }
  });
});
