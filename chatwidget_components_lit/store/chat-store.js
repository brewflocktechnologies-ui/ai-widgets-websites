import { getParentTheme, isHostDark, observeDarkMode } from '../utils/theme.js';
...
function buildDefaultBubble(theme) {
    return {
...
function buildDefaultGreetWindow(theme) {
    return {
        enabled: false,
        dismissed: false,
...
function buildDefaultChatWindow(theme) {
    return {
        useWebsiteTheme: true,
        offsetRight: null,
        offsetBottom: null,
...
function buildDefaultChatbar(theme) {
    return {
        enabled: false,
        useWebsiteTheme: true,
        position: 'bottom-right',
...
function buildDefaultChat(chatConfig) {
...
export const bubbleStore = { get() { return getStore().bubble; } };
export const greetWindowStore = { get() { return getStore().greetWindow; } };
export const chatWindowStore = { get() { return getStore().chatWindow; } };
export const featuresStore = { get() { return getStore().features; } };
export const chatbarStore = { get() { return getStore().chatbar; } };
export const chatStore = {
    get() { return getStore().chat; },
...
export async function initStore() {
    ...
    try {
        ...
        if (chatbarConfig && Object.keys(chatbarConfig).length > 0) {
            ...
            if (cb.layout === 'card') {
                if (cb.cardOffsetRight !== undefined && cb.cardOffsetRight !== null)
                    cb.offsetRight = cb.cardOffsetRight;
                if (cb.cardOffsetBottom !== undefined && cb.cardOffsetBottom !== null)
                    cb.offsetBottom = cb.cardOffsetBottom;
            }
            else {
                if (cb.barOffsetRight !== undefined && cb.barOffsetRight !== null)
                    cb.offsetRight = cb.barOffsetRight;
                if (cb.barOffsetBottom !== undefined && cb.barOffsetBottom !== null)
                    cb.offsetBottom = cb.barOffsetBottom;
            }
            emit('store:chatbar');
        }
        ...
        if (chatConfig && Object.keys(chatConfig).length > 0) {
            ...
            function applyTheme() {
                ...
                const active = JSON.parse(JSON.stringify(cc));
            ...
            if (cc.clientName)
                store.chat.clientName = cc.clientName;
            if (cc.agentName) {
                store.chat.agentName = cc.agentName;
                if (store.chat.messages[0])
                    store.chat.messages[0].senderName = cc.agentName;
            }
