...
export function getBorderRadius(borderRadius, fallback = '50%') {
...
export function getGradient(gradientType, gradientStops, gradientAngle = 135, fallbackColor = '#0b5fff') {
...
export function getBoxShadow(config) {
    const { boxShadowOffsetX = 0, boxShadowOffsetY = 8, boxShadowSpread = 0, boxShadowBlur = 20, boxShadowOpacity = 0.25, } = config;
...
export function getInnerShadow(config) {
    if (!config.innerShadow || !config.innerShadow.enabled)
        return '';
...
export function getCompositeBackground(config) {
    if (config.useWebsiteTheme)
        return config.backgroundColor || '#0b5fff';
    if (config.gradientType && config.gradientType !== 'none') {
        return getGradient(config.gradientType, config.gradientStops || [], config.gradientAngle ?? 135, config.backgroundColor || '#0b5fff');
    }
    return config.backgroundColor || '#0b5fff';
}
// ---------------------------------------------------------------------------
...
export function getChatbarBackground(config) {
    if (config.useWebsiteTheme)
        return config.accentColor || '#0b5fff';
...
// ---------------------------------------------------------------------------
export function getChatbarFontSize(textSize = 14, height = 40) {
    return Math.min(textSize, Math.max(12, Math.floor(height * 0.35))) + 'px';
}
export function getChatbarIconWidth(iconWidth = 20, height = 40, type = 'lucide') {
    return Math.min(iconWidth, Math.max(16, Math.floor(height * (type === 'customSvg' ? 0.55 : 0.5))));
}
export function getChatbarIconHeight(iconHeight = 20, barHeight = 40, type = 'lucide') {
    return Math.min(iconHeight, Math.max(16, Math.floor(barHeight * (type === 'customSvg' ? 0.55 : 0.5))));
}
// ---------------------------------------------------------------------------
// Tooltip style helpers (bubble)
// ---------------------------------------------------------------------------
export function getTooltipBorderRadius(borderRadius, pos) {
...
export function getAnimClass(animation) {
